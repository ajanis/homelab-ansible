...
def main() -> None:
    ...
    if module.check_mode:
        exit_module(
            module=module, out="", rc=0, cmd=[], err="", startd=startd, changed=False
        )
    ...
    if state == "present":
        ...
        if name in current_names:
            current_state_host = [
                host for host in current_state if host["hostname"] == name
            ][0]
            ...
            if differences:
                ...
                for diff in differences:
                    if diff in current_state_host["labels"]:
                        ...
                        action = "add"
                    rc, cmd, out, err = update_label(
                        module, action, current_state_host["hostname"], diff
                    )
                    _out.append(diff)

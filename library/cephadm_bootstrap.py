...
def main() -> None:
    module = AnsibleModule(
        ...
        required_together=[("registry_url", "registry_username", "registry_password")],
    )
    ...
    if fsid:
        if os.path.exists(os.path.join(data_dir, fsid)):
            ...
            exit_module(
                rc=0, startd=startd, module=module, cmd=cmd, out=out, changed=False
            )

    for f in [ceph_conf, ceph_keyring, ceph_pubkey]:
    ...
    if module.check_mode:
        exit_module(
            module=module, out="", rc=0, cmd=cmd, err="", startd=startd, changed=False
        )
    else:
        rc, out, err = module.run_command(cmd)
        exit_module(
            module=module, out=out, rc=rc, cmd=cmd, err=err, startd=startd, changed=True
        )
